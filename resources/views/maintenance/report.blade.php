...
<table class="heades" style="margin-bottom: 10px;width:100%;">
    <tr>
      ...
      <table class="lside" style="margin-bottom: 10px;">
          <tr>
            <td>Denpasar, {{ date("d/M/Y",strtotime($maintenance[0]->date))}}</td>
          </tr>
        ...
        <table style="margin-bottom: 10px;width: 100%;">
          <tr>
            <td style="text-align: center;">
                <u>Report Maintenance</u><br>
                Nomor : {{$maintenance[0]->nomaintenance}}<br>
    ...
    <tr>
      <td width="40%">
        ...
        <tr>
            ...
            <td width="10%">:</td>
            <td width="30%">{{$maintenance[0]->customer}}</td>
          </tr>
          <tr>
            <td width="30%">Alamat</td>
            <td width="10%">:</td>
            <td width="30%">{{$maintenance[0]->address}}, {{ $maintenance[0]->city }} </td>
          </tr>
          <tr>
            <td width="30%">Contact </td>
            <td width="10%">:</td>
            <td width="30%">{{$maintenance[0]->contact}} (mobile : {{$maintenance[0]->mobile}})</td>
          </tr>   
          <tr>
            <td width="30%">Problem</td>
            <td width="10%">:</td>
            <td width="30%">{{$maintenance[0]->problem}}</td>
          </tr> 
        </table>
      </td>
      <td width="10%"></td>
      <td width="40%">
        ...
        <tr>
            ...
            <td width="10%">:</td>
            <td width="30%">{{$maintenance[0]->teknisia}} {{$maintenance[0]->teknisib}}</td>
          </tr>
          <tr>
            <td width="30%">IP Address</td>
            <td width="10%">:</td>
            <td width="30%">{{$maintenance[0]->ips}}</td>
          </tr>
          <tr>
            <td width="30%">POPs</td>
            <td width="10%">:</td>
            <td width="30%">{{$maintenance[0]->pops}}</td>
          </tr>
          
          <tr>
            <td width="30%">Result</td>
            <td width="10%">:</td>
            <td width="30%">{{$maintenance[0]->result}}</td>
          </tr>
...
<table class="body_table " style="margin-bottom: 10px;width:100%;">
  <tr>
      <td colspan="4">Revocation List</td>
  ...
  @foreach($installed as $ins)
    @if($ins->qty>0)
    <tr>
        <td style=" border: solid 1px #000;">{{$ins->stockcodename}}</td>
        <td style=" border: solid 1px #000;">{{$ins->stocknames}}</td>
        <td style=" border: solid 1px #000;">{{$ins->qty}} {{$ins->unit}}</td>
        
        <td style=" border: solid 1px #000;">
        @if($ins->type==1)
          List Installed Serial Number : <br>
          <?php
          $serial=explode(',',$ins->serial);
          foreach($serial as $noseri){
            if($noseri){
              echo '[V]'.$noseri.' &nbsp; &nbsp;';
            }
...
<!-- //End List Item -->
    ...
    <tr>
      <td width="30%"></td>
      <td width="39%"></td>
      <td width="30%">Denpasar, {{ date("d/M/Y",strtotime($maintenance[0]->date))}}<br>PT. TRIDATU NETWORK <br><br><br><br>{{$maintenance[0]->teknisia}} {{$maintenance[0]->teknisib}}<br> (Technian)</td>
    </tr>
